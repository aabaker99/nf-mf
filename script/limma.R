...
# https://raw.githubusercontent.com/greenelab/multi-plier/7f4745847b45edf8fef3a49893843d9d40c258cf/util/test_LV_differences.R
# Multi-PLIER LIMMA functions - {{
# J. Taroni 2018
...
parser$add_argument("--b-matrix", "-b")
parser$add_argument("--sample-meta", "-s")
parser$add_argument("--outdir", "-o")
args = parser$parse_args()

b.matrix = read.csv(args$b_matrix)
sample.info.df = read.csv(args$sample_meta)

# rename first column to 'Sample' to accomodate Multi-PLIER
b.matrix.colnames = colnames(b.matrix)
b.matrix.colnames[1] = 'Sample'
colnames(b.matrix) = b.matrix.colnames
sample.info.df.colnames = colnames(sample.info.df)
sample.info.df.colnames[1] = 'Sample'
colnames(sample.info.df) = sample.info.df.colnames

# b.matrix must be latent x sample
b.matrix = transpose.df(b.matrix)
row.names(b.matrix)

phenotype.col = 'tumorType'

limma.lv.list <- LVTestWrapper(b.matrix = b.matrix,
                               sample.info.df = sample.info.df,
                               phenotype.col = phenotype.col,
                               file.lead = "tmp", 
                               plot.dir = args$outdir,
